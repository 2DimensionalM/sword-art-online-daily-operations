...
Completed goals stay in the Signal Room archive after active goals and never appear in the homepage radar or goal ticker. Their completion state must remain legible without relying on color: a physical pin motif, struck title, explicit completion date, and reversible restore action form one system. The completed group may be hidden with a labeled count control; it is not a separate source of truth.

Completing an active goal is a high-reward transition. The control previews a lock-on state on hover and keyboard focus; activation sequences reticle acquisition, a directional impact, abstract paper-like fragments, a perspective card turn, the completion strike, and a final `GOAL COMPLETE` verdict. Supporting red, blue, green, and violet may flash only in the moving fragments while ink, paper, and signal yellow retain control of the composition. The effect blocks repeat input while the database update is in flight, fades within a few seconds, never adds sound, and collapses to the ordinary completion status when reduced motion is requested.
...
Signal dates use the shared in-product calendar treatment rather than the browser's native date popup. The compact panel uses square day cells, ink structure, a yellow selected state, explicit month navigation, and separate clear/today/confirm actions. It must cover the page-status footer and ticker while open, remain below the main menu, and preserve the same interaction and contrast in both goal and message editors.
